function [obj_val, x_return,y_return,z_return, result_info] = hsd_lqe(blk, At, c_input, b, rel_eps)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function solves problems of the following form
% min c'x s.t. Ax=b, x in K, where
...
% min sum_j (c(j)'x(j)) s.t. sum_j (A(j)x(j)) = b, x(j) in K(j), j=1,2,...,N
% where K(j) can be a nonnegative orthant of arbitrary dimension, a Lorentz cone, product of Lorentz
% cones, an exponential cone or product of exponential cones.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% x(5) in (K_exp)^7, x(6) in (R_+)^9, m=23.
...
max_iter_count = 100;
display(['Algorithm begins with size(A) = [' num2str(size(A,1)) ',' num2str(size(A,2)) '], density(A) = ' num2str(nnz(A)/(size(A,1)*size(A,2)))]);
for it_cout =1:max_iter_count
    ...
    G_bar = [G1; G2; G3]; % display(['density of G_bar = ' num2str(nnz(G_bar)/numel(G_bar))]);
