...
m = 2500;
blk{1,1} = 'l'; blk{1,2} = 1000; A_cell{1} = sprandn(m,sum(blk{1,2}), 0.12); c_cell{1} = randn(sum(blk{1,2}),1);
...
% [obj_lqeu, x_lqeu, y_lqeu, z_lqeu, info_lqeu] = hsd_lqeu_Schur_dense_column(blk, A_cell, c_cell,b, 1e-8);
[obj_lqeu, x_lqeu, y_lqeu, z_lqeu, info_lqeu] = hsd_lqeu_fast_lu(blk, A_cell, c_cell,b, 1e-8);
