...
% Basic model setting: there is ONE customer and 10 types of wines with score 1 to 10.
% Each time the customer gives an integer score between 1 and 10
% There have been N observations

% J is the number of product types
J = 10;

% Read the prucahse record data into a matrix
purchase_record_mat = csvread('data/wine_quality_white_matlab_friendly.csv',2);

% Take only a fraction of it since there are too many records
purchase_record_mat = purchase_record_mat(floor(end/80)*13+1:floor(end/80)*14,:);
...
% Construct N-dimensional vectors representing attribute values
    ...
    for j = 1:J
        for l = 1:J
            ...
            if l~=j
                ...
                x_nl = [disp_mat(n,l); feat_mat(n,l); log_price_mat(n,l)];
