function [obj_val, x_val, y_val] = solve_project_mgt_sdldr(C, beta, W, H, a, b, c, z_pos_mean)

% 
% +ROMETEST\SOLVE_PROJECT_MGT_SDLDR Helper routine to solve
% project management example using a segregated-deflected linear decision rule
%
%   [obj_val, x_val, y_val] = SOLVE_PROJECT_MGT_SDLDR(C, beta, W, H, a, b, c)
...
rome_begin;
h = rome_model('Project Management Example - SDLDR');
...
% apply DLDR bound
% [r, g] = RobustBounds.dldr_bound(num_edges, z, z_seg_LB, z_seg_UB);
% [r, g] = RobustBounds.dldr_bound(num_edges, z);

% try using other bound
r = rome_linearrule(num_edges, z);
r1 = rome_linearrule(num_edges, z);
r2 = r - r1;
g = rome_supp_bound(-r1) + rome_covar_bound(-r2);     % MTD 2
% sel_ind = 1:3;
% I = num_edges;
% g = RobustBounds.meanpositivebound(-r, z, I, sel_ind);  % MTD 3


% seqential constraint
dy1 = diff(y, [], 1); 
dy2 = diff(y, [], 2);
rome_constraint([dy1(:); dy2(:)] - r == b + a * (1 - x) .* (z1 + z2));

% budget constraint
rome_constraint(sum(c.*x(:)) <= C );

% objective
rome_minimize(strip_rand(y(H, W)) + sum(g));  %+ sum(strip_rand(w)));
